...
def test_print_str_writes_payload(h):
    h.call("screen_init")
    s = place_str(h, 0x7000, [ord("H"), ord("I")])
    h.rs_push(s)
    h.call("print_str")
...
def test_print_str_with_embedded_newline(h):
    ...
    s = place_str(h, 0x7000, [ord("A"), 0x0D, ord("B")])
    h.rs_push(s)
    h.call("print_str")
...
def test_print_int_renders_decimal(h):
    h.call("screen_init")
    # 1000 → "1000"
    x = place_int(h, 0x7000, [0xE8, 0x03])
    h.rs_push(x)
    h.call("print_int")
...
def test_println_str_adds_newline(h):
    h.call("screen_init")
    s = place_str(h, 0x7000, [ord("A"), ord("B")])
    h.rs_push(s)
    h.call("println_str")

...
def test_string_keys_sorted_lexicographically(h):
    d = h.alloc_dict()
    apple = place_str(h, 0x8900, [ord(c) for c in "apple"])
...
def test_mixed_type_keys(h):
    ...
    d = h.alloc_dict()
    int_key = place_int(h, 0x8900, [0x05])

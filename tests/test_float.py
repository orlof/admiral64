...
def test_unpack_to_fac1_one(hfp):
    ...
    f = place_python_float(hfp, 0x8A00, 1.0)
...
def test_unpack_to_fac1_zero(hfp):
    f = place_python_float(hfp, 0x8A00, 0.0)
    obj = hfp.read_word(f + H_PTR)
    hfp.write_word(0x14, obj + O_HEADER)
    hfp.call("_fp_unpack_to_fac1")
...
def test_pack_from_fac1_round_trip_one(hfp):
    ...
    hfp.write_word(0x14, 0x8A00)
    hfp.call("_fp_pack_from_fac1")
    assert hfp.read_bytes(0x8A00, 5) == [0x81, 0x00, 0x00, 0x00, 0x00]
...
def test_pack_from_fac1_neg_one(hfp):
    ...
    hfp.write_word(0x14, 0x8A00)
    hfp.call("_fp_pack_from_fac1")
    assert hfp.read_bytes(0x8A00, 5) == [0x81, 0x80, 0x00, 0x00, 0x00]
...
def test_pack_from_fac1_zero_canonical(hfp):
    ...
    hfp.write_word(0x14, 0x8A00)
    hfp.call("_fp_pack_from_fac1")
    assert hfp.read_bytes(0x8A00, 5) == [0, 0, 0, 0, 0]
...
def run_int_to_float(hfp, payload: list[int]) -> float:
    rsp_initial = hfp.rsp
    x = place_int(hfp, 0x8A00, payload)
    hfp.rs_push(x)
...
def run_float_to_int(hfp, value: float) -> list[int]:
    rsp_initial = hfp.rsp
    f = place_python_float(hfp, 0x8A00, value)
    hfp.rs_push(f)
...
def test_float_to_int_2_to_31(hfp):
    ...
    f = place_python_float(hfp, 0x8A00, 2_147_483_648.0)
    hfp.rs_push(f)
    hfp.call("float_to_int")
...
def test_float_to_int_2_to_32(hfp):
    ...
    f = place_python_float(hfp, 0x8A00, 4_294_967_296.0)
    hfp.rs_push(f)
    hfp.call("float_to_int")
    bs = read_int(hfp, hfp.read_word(RV))
...
def test_float_to_int_neg_3_billion(hfp):
    ...
    f = place_python_float(hfp, 0x8A00, -3_000_000_000.0)
    hfp.rs_push(f)
    hfp.call("float_to_int")
    bs = read_int(hfp, hfp.read_word(RV))
...
def test_float_to_int_one_e_12(hfp):
    ...
    f = place_python_float(hfp, 0x8A00, 1e12)
    hfp.rs_push(f)
    hfp.call("float_to_int")
    bs = read_int(hfp, hfp.read_word(RV))
...
def test_float_to_int_just_below_overflow(hfp):
    ...
    f = place_python_float(hfp, 0x8A00, 2_147_483_647.0)
    hfp.rs_push(f)
    hfp.call("float_to_int")
    bs = read_int(hfp, hfp.read_word(RV))
...
def run_binop(hfp, op_label: str, left: float, right: float) -> float:
    rsp_initial = hfp.rsp
    a = place_python_float(hfp, 0x8A00, left)
...
def test_div_by_zero_panics(hfp):
    rsp_initial = hfp.rsp
    a = place_python_float(hfp, 0x8A00, 1.0)
...
def test_val_cmp_floats_gt(hfp):
    a = place_python_float(hfp, 0x8A00, 2.0)
    b = place_python_float(hfp, 0x9100, 1.0)

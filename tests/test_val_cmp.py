...
def test_int_eq(h):
    a = place_int(h, 0x8800, [0xE8, 0x03])
...
def test_int_gt(h):
    a = place_int(h, 0x8800, [0x02])
    b = place_int(h, 0x9100, [0x01])
    assert run_cmp(h, a, b) == 1


def test_neg_int_lt_pos(h):
    a = place_int(h, 0x8800, [0xFF])  # -1
    b = place_int(h, 0x9100, [0x01])
    assert run_cmp(h, a, b) == -1
...
def test_string_keys_sortable(h):
    ...
    apple = place_str(h, 0x8800, [ord("a"), ord("p"), ord("p"), ord("l"), ord("e")])

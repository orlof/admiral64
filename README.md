...
Admiral64 is a self-hosted environment that boots from a `.PRG` on disk and gives you:
 ...
 - an interactive REPL with line editing and auto-printing of expressions (BASIC-style `PRINT X,` suppresses the newline),
 ...
 - hi-res (320×200) and multicolor (160×200) bitmap graphics via a `BITMAP()` memory-mode switch plus on-disk drawing libraries (`hires`, `mc`, `text`) loaded as ready-to-use dicts.
...
This is a personal-fun project, not a polished product:
 ...
 - Bitmap graphics exist (hi-res + multicolor) but switching into them is a `BITMAP()` into a smaller-heap memory config, not a live mode flip. No sprites, no sound, no virtual memory, no multitasking. Just enough to play with the language.
